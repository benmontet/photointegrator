...
class NBodyState{
 private:
  ...
  double * radii;
  double * Prot;  // rotation period
  double * rotation;
  double * k2;   // apsidal motion constant
  double * k2val;
  double time;
  int status;
  int N;
 ...
 public:
  ...
  NBodyState(double * ms, double * a, double * e, double * in, double * o,double * ln, double * m, int NN, double t0, double * rad, double * rotation, double * k2val);
  ...
  // Returns radius of object obj
  double getRadius(int obj);

...
NBodyState::NBodyState(double * m, double posj[][3], double velj[][3], int NN,double t0) {
  ...
  evolve(rj,vj,aj,mass,eta,N,radii,Prot,k2,t0,t0,1,status,0,0);
...
NBodyState::NBodyState(double * ms, double * a, double * e, double * in, double * o,double * ln, double * m, int NN, double t0, double * rad, double * rotation, double * k2val) {
  N = NN;
  mass = new double[N];
  radii = new double[N];
  Prot = new double[N];
...
double NBodyState::operator() (double t, double H, double ORBIT_ERROR, double HLIMIT) {
  evolve(rj,vj,aj,mass,eta,N,radii,Prot,k2,time,t,H,status,ORBIT_ERROR,HLIMIT); time = t;
  bary_coords();
...
double NBodyState::getRadius(int obj) {return radii[obj]; }

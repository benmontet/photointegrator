...
int main(int argc, char * argv[]) {
  ...
  double mass[N],radii[N],flux[N],u1[N],u2[N],Prot[N],k2[N];
  ...
  NBodyState state(mass,a,e,inc,om,ln,ma,N,t0,radii,Prot,k2);
  ...
  cout <<"#Index\t|\tMass\t\tRadius\t\tFlux\t\tu1\t\tu2\t\tProt\t\tk2" << endl;
  for (int i = 0; i < N; i++) {
    cout << "# " << i << "\t|\t" << mass[i] << "\t" << radii[i] << "\t" << flux[i] << "\t"
	 << u1[i] << "\t" << u2[i] << "\t" << Prot[i] << k2[i] << endl;
  } 
  cout << "#\n#At Time = " << t0 << endl;
